...
export async function POST(request: NextRequest) {
  ...
  const authorName = normalizeName(payload.authorName);
  ...
  await db.prepare(
    `insert into comments
      (id, thread_id, parent_id, author_name, body, score, ip_hash, user_agent, created_at, updated_at)
    values (?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
  ).bind(id, threadId, parentId, authorName, body, hash, userAgent, now, now).run();
